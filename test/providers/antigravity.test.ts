...
const fixtureDir = join(import.meta.dirname, "..", "fixtures", "antigravity");
const originalAgyBinary = process.env[ANTIGRAVITY_BINARY_ENV];
...
fs.writeFileSync(process.env.ANTIGRAVITY_RECORD, JSON.stringify({
...
`,
  ...
  it("falls back through current-user home sources when HOME is absent or empty", async () => {
    ...
    const executable = fakeExecutable(
      `
const fs = await import("node:fs");
fs.writeFileSync(process.env.ANTIGRAVITY_RECORD, JSON.stringify(process.env.HOME));
process.stdout.write(JSON.stringify(${JSON.stringify(fixture("usage-empty.json"))}));
`,
      { ANTIGRAVITY_RECORD: record },
    );
  ...
  it("classifies auth stderr without exposing it and never reads credentials", async () => {
    ...
    process.env[ANTIGRAVITY_BINARY_ENV] = executable;
  ...
  it("waits for a stubborn child to exit before cleanup and returning", async () => {
    ...
    const executable = fakeExecutable(
      `
const fs = await import("node:fs");
fs.writeFileSync(process.env.ANTIGRAVITY_RECORD, JSON.stringify({ cwd: process.cwd(), pid: process.pid }));
process.on("SIGTERM", () => {});
setInterval(() => {}, 1000);
`,
      { ANTIGRAVITY_RECORD: record },
