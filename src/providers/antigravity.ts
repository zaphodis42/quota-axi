...
export async function resolveAgyBinary(): Promise<AgyBinaryState> {
  const configured = process.env[ANTIGRAVITY_BINARY_ENV];
...
export async function runAgyCommand(options: {
  ...
  const timeoutMs = options.timeoutMs ?? ANTIGRAVITY_TIMEOUT_MS;
  const maxOutputBytes = options.maxOutputBytes ?? ANTIGRAVITY_MAX_OUTPUT_BYTES;
