...
- It reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.ai Coding Plan, and Antigravity quota windows, and it must never route, recommend, proxy, intercept, log in, import browser cookies, or mutate provider state.
...
This fork publishes as `@zaphodis42/quota-axi` (scoped, `publishConfig.access: public`), independent of the upstream `quota-axi` package on npm. Releases are cut by release-please from conventional commit messages on `main`; merging the bot's release PR triggers `npm publish` via `.github/workflows/release-please.yml`, using npm's OIDC trusted-publisher flow (`id-token: write` + `--provenance`), not an `NPM_TOKEN` secret. The npm Trusted Publisher must be configured on npmjs.com for `@zaphodis42/quota-axi` pointing at the `zaphodis42/quota-axi` GitHub repo and the `release-please.yml` workflow file.
`.release-please-manifest.json` is primed at `0.1.0` since `@zaphodis42/quota-axi` has never been published under this scoped name; release-please owns every version after that.
`release-please-config.json` sets `bootstrap-sha` to `898189cba57912f651b5375bb141ee22543945a3`, the commit this fork's release automation was retargeted from `quota-axi` (kunchenguid) to `@zaphodis42/quota-axi`; do not retarget it further unless the published baseline itself is being corrected.
